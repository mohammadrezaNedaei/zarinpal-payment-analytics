...
- ⭕ **اتصال Backend واقعی (قدم بعدی برنامه‌ریزی‌شده):** هم‌تیمی کاربر بخش Backend را طبق BACKEND_IMPLEMENTATION_SPEC.md آماده می‌کند؛ کاربر ریپو را clone می‌کند و مسیر/آدرس را می‌دهد؛ سپس فقط `src/api/adapter.ts` با fetch به API واقعی بازنویسی می‌شود (مرجع مسیرها در README و spec بخش ۱۴). قراردادهای `types.ts` از قبل با spec هماهنگ‌اند و هیچ تغییری در صفحات لازم نیست.
